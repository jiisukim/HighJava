...
public class T06_TreeSetTest {
	...
	public static void main(String[] args) {
		...
		// => headSet(기준값) 메서드 (기본적으로 '기준값'은 포함 시키지 않음)
		// => headSet(기준값, 논리값) (논리값이 true이면 '기준값'을 포함함)
		SortedSet<String> ss1 = ts.headSet("K"); // 아무것도 안쓰면 false 생략
		System.out.println("K 이전 자료 :" + ss1); //기준 값 미포함 
		System.out.println("K 이전 자료(기준 값 포함) : "
					+ ts.headSet("K", true));
		
		// '기준값' 보다 큰 자료를 찾아 SortedSet으로 반환하는 메서드
		// tailSet(기존값) => 기본적으로 '기준값'을 포함시킨다.
		// tailSet(기준값, 논리값) => 논리값이 false이면 '기준값'을 포함시키지 않음
		SortedSet<String> ss2 = ts.tailSet("K"); // 아무것도 안쓰면 true 생략, 기준값도 포함해 나옴
		System.out.println("K 이후 자료 : "+ ss2);
		System.out.println("K 이후 자료 (기준가 미포함): "+ ts.tailSet("K",false));
		
		//subSet(기준값1,기준값2) => 기준값 1~ 기준값2 사이의 값을 가져온다.
		//						('기준값1' 포함, '기준값2' 미포함)
		//subSet(기준값1,논리값1,기준값2,논리값2)
		// => 각 기준값을 포함할지 여부를 설정한다.(논리값이 true이면 포함 아니면 불포함)
