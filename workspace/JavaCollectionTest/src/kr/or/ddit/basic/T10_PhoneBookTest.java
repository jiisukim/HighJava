...
public class T10_PhoneBookTest {
	...
	private void search() {
		System.out.println();
		System.out.println("새롭게 검색할 전화번호 정보를 입력하세요");
		System.out.print("이름>> ");
	...
	private void delete() {
		System.out.println();
		System.out.println("새롭게 삭제할 전화번호 정보를 입력하세요");
		System.out.print("이름>> ");
		String name = scan.next();
	...
	private void update() {
		...
		// 이미 등록된 사람인지 검사
		// get()메서드로 가져올 때 가져올 자료가 없으면 null을 반환
		...
		scan.nextLine(); // 입력버퍼에 남아있는 엔터키 값을 읽어오는 역할을 수행함.
							// next() 후 혹시 남아있을지도 모를 쓰레기 값 제거용.
