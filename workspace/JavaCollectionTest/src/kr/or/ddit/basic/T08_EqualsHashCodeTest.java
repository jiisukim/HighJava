 ...
 * 해시함수에 의해 얻어지는 값은 해시 값, 해시 코드, 해시 체크섬 또는 간단하게 해시라고한다.
 * 
 ...
 * 1. 두 객체가 같으면 반드시 hashCode를 가져야한다.
 ...
 * 4. equals()메서드를 override 하면 반드시 hashCode()메서드도 override해야한다.
 ...
 * 
 * 	hashCode()메서드에 사용하는 '해싱알고리즘'에서 서로다른 객체에 대하여 같은 hashcode값을 만들어 낼 수 있다. 그래서 객체가같지 않더라도 hashcode가 같을 수 있다.
 * 
 */
	// 해시 코드 : 주소값이 같은지 //equals : 객체의 값이 같은지
	public static void main(String[] args) {
	Person p1 = new Person(1,"홍길동");
	Person p2 = new Person(1,"홍길동");
	Person p3 = new Person(1,"변학도");
	
	System.out.println("p1.equals(p2) : "+ p1.equals(p2)); //p1과 p2는 다른객체 필드 값만 같을 뿐
	...
	System.out.println("add(p3) 성공후 데이터");
	for(Person p : set) {
		System.out.println(p.getId()+" : "+ p.getName());
	}
...
class Person{
	...
	public boolean equals(Object obj) {
		...
		if (obj == null)
			return false;
		if (getClass() != obj.getClass()) //getClass : 현재 객체가 만들어질때 기반이 되는 클래스
			return false;
