...
//		2. new File(File parent, String child) ==> 'parent' 디렉토리 안에 있는 'child'파일 또는 디렉토리를 갖는다.
		...
		System.out.println(file3.getName() + "의 용량 크기 : "+ file3.length() + "byte");
		 ...
		 * 
		 * 디렉토리 폴더 만들기
		 * 1. mkdir() => File객체의 경로 중 마지막 위치의 디렉토리를 만든다. 중간의 경로가 모두 미리 만들어져 있어야한다.
		 * 2.  mkdirs() = > 중간의 경로가 없으면 중간의 경로도 새롭게 만든 후 마지막 위치의 디렉토리를 만들어준다.
		 * => 위 두 메서드 모두 만들기를 성공하면 true, 실패하면 false .
		 * 
		 */
