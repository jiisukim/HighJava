...
class Member implements Serializable{
   /*
       자바는 Serializable 인터페이스를 구현한 클래스만 직렬화 할 수 있도록 제한 하고 있음.
       구현하지 않으면 직렬화 작업시 java.io.NotSerializableException 예외 발생함.
    */
   /*
       transient = > 직렬화가 되지 않을 멤버변수에 지정한다.
                (*static 필드도 직렬화 대상이 아니다.)
       직렬화가 되지 않는 멤버변수는 기본값으로 저장된다.
       (참조형 변수: null, 숫자형변수: 0)                  //인스턴스 변수에 담긴 정보만 직렬화 가능하다
    */
   private String name;
   private int age;
   private String addr;
