 ...
 *  2.공통으로 사용할 수있는 클래스와 인스턴스를 만든다
 *  3. 이 인스턴스를 각각의 스레드에 넘겨준다.
 *  4. 각각의 스레드는 이 인스턴스 참조값을 저장하는 변수를 이용하여 공통데이터를 사용하한디.
 *  
 *  ex)원주율을 게산하는 스레드가 있고, 계산된 위원주율을 출력하는 스레드가 있다.
 *  원주율을 계산후 이 값을 출력하는 프로그램 작성하기
 */
...
class ShareData{
	...
	/*
	 *  volatile => 선언된 변수를 컴파일러의 최적화 대상에서 제외시킵니다.
...
class CalcPIThread extends Thread{
	...
	for(int i =1; i<= 1500000000; i+=2) {
		...
		sd.result = sum*4; //계산된 원주율 공통객체 멤버변수에 저장
