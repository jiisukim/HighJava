...
class Util{
	/**
	 * 제너릭 메서드 <T,R> R R method(T t)
	 * => 파라미터 타입과 리턴타입으로 타입 파라미터를 가지는 메서드
	 * 	선언방법 : 리턴타입 앞에 <> 기호를 추가하고 타입 파라미터를 기술후 사용한다.
	 * @param p1
	 * @param p2
	 * @return
	 */
...
class Pair<K,V>{
	...
	//키와 값을 출력하기
	public <K,V> void displayAll(K key, V val) {	//여기서 쓴 K, V는 지역변수 느낌 
		System.out.println(key.toString()+" : "+ val.toString());
	}
	
}
public class T13_GenericMethodTest {
	public static void main(String[] args) {
		...
		if(result2) {
			...
			System.out.println("논리적으로 동일한 객체가 아님");
		}
		// 제너릭 메서드 테스트
