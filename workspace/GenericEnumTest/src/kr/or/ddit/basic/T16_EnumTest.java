 ...
 */
	...
	public enum City{서울,부산,대구,광주,대전};  //city 타입이라는 enum타입
	
	// 데이터 값을 임의로 지정한 열겨형 객체 선언
	// 데이터 값을 정해 줄 경우에는 생성자를 만들어서 괄호속의 변수에 저장되도록 해야 한다.
	public enum Season{
		...
		Season(String data) { // private 앞에 생략되어있음 private Season(String data)  
			this.str = data;  // 생성자 private라서 객체생성이 하나밖에 안됨
		}
		//값을 반환하는 메서드
	...
	public static void main(String[] args) {
		 ...
		 *2. ordinal() => 열거형 상수가 정의된 순서값을 반환한다.(기본적으로 0부터시작)                                        
		 *3. valueOf("열거형상수이름") => 지정된 열거형에서 '열거형 상수 이름'과 일치하는 열거형 상수를 반환한다.
		 */
		...
		// 열거형 객체변수에 값 저장하기
		myCity2 = City.서울;
		myCity1 = City.valueOf("서울"); // City enum에서 '서울'데이터 가져오기
